...
def paramsTocsv(data_name, scaler, model_name, best_params, best_score, testset_score):
    now = datetime.now()
    # 폴더가 없으면 생성 있으면 통과
    try:
    ...
    if os.path.isfile('./modeldata/' + data_name + '.csv'): # 파일 존재
        ...
        df_nonparam = pd.DataFrame([{'date':date,'model_name':model_name,'scaler':scaler,'best_score':best_score, 'testset_score':testset_score}]) #시간 + score 데이터
